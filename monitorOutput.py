					...
					if len(data3["people"])!=0:
						...
						with open((outputLocation+"/"+sample)) as f:
							...
							for i in range(len(data1["people"])):
								beforeFrameList.append(temp)
								myDistList.append(temp)

							for z in range(len(data1["people"])):			
								for y in range(len(data1["people"][z])):
									beforeFrameList[z]=data1["people"][z]["pose_keypoints_2d"]
						with open((outputLocation+"/"+previousSample)) as f:
							data = json.load(f)
						...
						mainlist=myDistList
						for z in range(len(mainlist)):
							if(mainlist):	
								single_test = np.array(mainlist[z])
